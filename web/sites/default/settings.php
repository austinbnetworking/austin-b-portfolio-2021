...
/**
 * Load dotenv environment config sync directory.
 */
if (!array_key_exists('CONFIG_SYNC_DIR', $_SERVER) || empty($_SERVER['CONFIG_SYNC_DIR'])) {
 $config_sync_dir = '../config/sync';
}
else {
 $config_sync_dir = $_SERVER['CONFIG_SYNC_DIR'];
}
$settings['config_sync_directory'] = join(DIRECTORY_SEPARATOR, [DRUPAL_ROOT . DIRECTORY_SEPARATOR . $config_sync_dir]);

/**
 * Load dotenv environment hash salt.
 */
$salt = $_SERVER['HASH_SALT'];
if (file_exists($salt) !== FALSE) {
 $salt = file_get_contents(join(DIRECTORY_SEPARATOR, [DRUPAL_ROOT . DIRECTORY_SEPARATOR . $salt]));
}
$settings['hash_salt'] = $salt;

/**
 * Load dotenv environment trusted hosts.
 */
$trusted_hosts = $_SERVER['TRUSTED_HOSTS'];
if (!empty($trusted_hosts)) {
 $file_trusted_hosts = join(DIRECTORY_SEPARATOR, [DRUPAL_ROOT . DIRECTORY_SEPARATOR . $trusted_hosts]);
 if (file_exists($file_trusted_hosts) !== FALSE && ($trusted_hosts = file_get_contents($file_trusted_hosts)) !== FALSE) {
   $trusted_hosts = explode(PHP_EOL, trim($trusted_hosts));
 }
 else {
   $trusted_hosts = NULL;
 }
...
/**
 * Load dotenv database environment variables.
 */
$databases['default']['default'] = array (
  'database' => $_SERVER['DATABASE_NAME'],
  'username' => $_SERVER['DATABASE_USER'],
  'password' => $_SERVER['DATABASE_PASSWORD'],
  'prefix' => $_SERVER['DATABASE_PREFIX'] ?? NULL,
